...
DRIVE_ROOT="/content/drive/MyDrive/university_of_copenhagen/master_thesis/poli_benchmark/results_iterations"
...
for seed in {1..5}; do
  run_one "osimetrinib_mpo"   "${seed}" 15 40 || status=1
  sync_results

  run_one "median_2"         "${seed}" 15 40 || status=1
  sync_results

  run_one "amlodipine_mpo"  "${seed}" 15 40 || status=1
  sync_results

  run_one "perindopril_mpo"               "${seed}" 15 40 || status=1
  sync_results

  run_one "ranolazine_mpo"               "${seed}" 15 40 || status=1
  sync_results

  run_one "zaleplon_mpo"               "${seed}" 15 40 || status=1
  sync_results
done
